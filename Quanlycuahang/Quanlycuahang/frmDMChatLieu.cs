...
namespace Quanlycuahang
{
    ...
    {
        DataTable tblCL; //khai bao bien table
        ...
        private void txtMaChatLieu_Click(object sender, EventArgs e)
        {
        ...
        {
            txtMaChatLieu.Enabled = false;      //không cho điền vào chỗ trống
            btnLuu.Enabled = false;             // ẩn 2 button 
        ...
        {
            if (btnThem.Enabled == false) // nếu nút thêm không thể ấn 
            {
                MessageBox.Show("Đang ở chế độ thêm mới!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtMaChatLieu.Focus(); //focus con trỏ vào ô mã chất liệu
                return;
            }
        ...
        private void btnBoQua_Click(object sender, EventArgs e)  //Hủy chức năng thêm mới hoặc sửa
        {
            ResetValue();
            btnBoQua.Enabled = false;
            btnThem.Enabled = true;
        ...
        {
            if (e.KeyCode == Keys.Enter)            //Nếu ấn enter sẽ nhận tín hiệu tab xuống dòng
                SendKeys.Send("{TAB}");
        }
