...
using System.Windows.Forms;     // Sử dụng MessageBox
...
namespace Quanlycuahang.Class
{
    class Functions
    {
        ...
        public static void Connect()       //Tạo phương thức Connect()
        {
            ...
            con.ConnectionString = Properties.Settings.Default.QLCuaHang;
        ...
        public static void Disconnect()     //Tạo phương thức Disconnect
        {
        ...
        public static DataTable GetDataToTable(string sql)      //Tạo phương thức lấy dữ liệu vào bảng
        {
            ...
            dap.Fill(table);            //Lấy kết quả từ câu lệnh sql để điền vào table
            return table;
        }
        public static void RunSQL(string sql)       //Tạo phương thức thực thi các câu lệnh sql  insert , update , delete
        {
            SqlCommand cmd; //Khởi tạo đối tượng cmd thuộc lớp SqlCommand
            cmd = new SqlCommand();
            cmd.Connection = con; //Gán kết nối
            cmd.CommandText = sql; //Gán lệnh SQL
            try         //cấu trúc try ... catch nếu gặp lỗi nhảy => catch
            {
                cmd.ExecuteNonQuery(); //Thực hiện câu lệnh SQL không truy vấn 
            }
            catch (Exception ex)
            {
        ...
        public static bool CheckKey(string sql)         //Hàm kiểm tra xem khoá có trùng không
        {
            DataTable table = new DataTable();          //Khai báo đối tượng table thuộc lớp DataTable
            SqlDataAdapter dap = new SqlDataAdapter(sql, con);      //Định nghĩa đối tượng thuộc lớp SqlDataAdapter
            dap.Fill(table);            //Lấy kết quả từ câu lệnh sql để điền vào table
        ...
        public static string CreateKey(string tiento)       //Tạo mã HD theo quy luật : TientoNgaythangnam_giophutgiay
        {
            string key = tiento;
            string[] partsDay;
        ...
        public static string ConvertTimeTo24(string hour)       //chuyển sang dạng thời gian 24 giờ
        {
            string h = "";
            switch (hour)
            {
